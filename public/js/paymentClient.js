...
function openPayment(sel, domain, formId) {
  ...
  if (sel == "1") {
    ...
    if (desc == null || desc.value == "" || desc.value == null) {
      alert(
        "description should not be empty or undefined. please check this field in your form"
      );
      return;
    }
    //
  ...
  var payOption = !!paymentForm.elements["payOption"]
    ? paymentForm.elements["payOption"].value
    : "";
  ...
  if (
    !!paymentForm.elements["payType"] &&
    paymentForm.elements["payType"].value == "EW" &&
    !!paymentForm.elements["bankCode"] &&
    paymentForm.elements["bankCode"].value == "MOMO"
  ) {
  ...
  if (
    ...
    paymentForm.elements["payType"].value == "EW" &&
    !!paymentForm.elements["bankCode"] &&
    paymentForm.elements["bankCode"].value == "MOCA"
  ) {
  ...
  if (
    ...
    paymentForm.elements["payType"].value == "EW" &&
    !!paymentForm.elements["bankCode"] &&
    paymentForm.elements["bankCode"].value == "SHPP"
  ) {
  ...
  if (
    ...
    paymentForm.elements["payType"].value == "EW" &&
    !!paymentForm.elements["bankCode"] &&
    paymentForm.elements["bankCode"].value == "VTTP"
  ) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      paymentForm.elements["ipinfo"].value =
        data.substring(data.indexOf("ip"), data.indexOf("ts")) +
        "&" +
        data.substring(data.indexOf("loc"), data.indexOf("tls"));
    },
    complete: function () {
      if (
        paymentForm.elements["merId"].value == "SAMSUNG001" ||
        paymentForm.elements["merId"].value == "SAMSUNG002" ||
        paymentForm.elements["merId"].value == "SAMSUNG003" ||
        paymentForm.elements["merId"].value == "SAVINA0001" ||
        paymentForm.elements["merId"].value == "SAVINA0002" ||
        paymentForm.elements["merId"].value == "SAVINA0003"
      ) {
        windowType = "";
      }
      if (windowType != null && windowType != "") {
        ...
        if (windowType == "0") {
          ...
          if (
            ((payType == "NO" || payType == "EQ") &&
              (!paymentForm.elements["payOption"] ||
                paymentForm.elements["payOption"].value != "PURCHASE_OTP")) ||
            (payType == "EW" &&
              (paymentForm.elements["bankCode"] == undefined ||
                paymentForm.elements["bankCode"].value == ""))
          ) {
        ...
        paymentFormJquery.append(
          "<input type ='hidden' name='mobileCheck' value=''>"
        );
        paymentForm.elements["mobileCheck"].value = navigator.platform;
        if (navigator.platform) {
          paymentForm.elements["mobileCheck"].value +=
          ...
          paymentFormJquery.append(
            "<input id='checkIpad' type ='hidden' name='checkIpad' value=''>"
          );
...
function init(paymentForm) {
  ...
  if (div_paymentLayer == null || div_paymentLayer == undefined) {
    ...
    innerData +=
      '<iframe name="paymentF" allow="payment" id="paymentF" width="100%" height="100%" marginWidth="0" marginHeight="0" frameBorder="0" scrolling="no" style="background-image: url(../images/progress.gif); background-repeat: no-repeat; background-position: center center;" ></iframe>';
    innerData += "</div>";
    innerData += "</div>";
    innerData += "</div>";
    innerData += "</div>";
...
function deleteToken() {
  ...
  if (
    (deleteForm.payToken.value != "" && deleteForm.payToken.value != null) ||
    (deleteForm.merId.value != "" && deleteForm.merId.value != null) ||
    (deleteForm.payType.value != "" && deleteForm.payType.value != null)
  ) {
    makeMerchantToken("delete");
    $.post(
      "/pg_was/deleteTokenAPI.do",
      $("#deleteTokenForm").serialize(),
      function (data) {
        var param =
          "trxId=&merId=" +
          data.merId +
...
function isOldDevice() {
  if (
    ...
    (isiPad(window) && checkMaxAnisotropy() < 4)
  ) {
    return true;
  } else {
    return false;
